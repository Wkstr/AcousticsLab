#pragma once
#ifndef SOUND_CLASSIFICATION_HPP
#define SOUND_CLASSIFICATION_HPP
...
static inline std::shared_ptr<module::MDAG> createSoundClassification(const core::ConfigMap &configs)
{
    ...
    auto output_node = module::MNodeBuilderRegistry::getNode("output", configs, &inference_outputs, &inference_outputs);
    if (!output_node)
    {
        return nullptr;
    }
    ...
    if (!dag->addEdge(input_node.get(), feature_node.get()) || !dag->addEdge(feature_node.get(), inference_node.get())
...
#endif // SOUND_CLASSIFICATION_HPP