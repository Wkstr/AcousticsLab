...
int lfs_flashbd_create(const struct lfs_config *cfg, const struct lfs_flashbd_config *bdcfg)
{
    ...
    LFS_FLASHBD_TRACE("lfs_flashbd_create(%p {.context=%p, "
        ...
        (void *)(uintptr_t)cfg->erase, (void *)(uintptr_t)cfg->sync, (void *)bdcfg, bdcfg->read_size, bdcfg->prog_size,
        bdcfg->erase_size, bdcfg->erase_count, bdcfg->flash_addr);
    ...
    if (!default_littlefs_partition)
    {
        ...
        if (!default_littlefs_partition)
        {
            LFS_ASSERT(default_littlefs_partition && "Partition not found");
            return LFS_ERR_INVAL;
        }
    }

    bd->flash_addr = (void *)default_littlefs_partition->address;
...
int lfs_flashbd_read(const struct lfs_config *cfg, lfs_block_t block, lfs_off_t off, void *buffer, lfs_size_t size)
{
    ...
    int ret = esp_partition_read(default_littlefs_partition, block * bd->cfg->erase_size + off, buffer, size);

    LFS_FLASHBD_TRACE("lfs_flashbd_read -> %d", size);
    return ret == ESP_OK ? 0 : LFS_ERR_IO;
}

int lfs_flashbd_prog(const struct lfs_config *cfg, lfs_block_t block, lfs_off_t off, const void *buffer,
    lfs_size_t size)
{
    ...
    int ret = esp_partition_write(default_littlefs_partition, block * bd->cfg->erase_size + off, buffer, size);
...
{
    LFS_FLASHBD_TRACE("lfs_flashbd_erase(%p, 0x%" PRIx32 " (%" PRIu32 "))", (void *)cfg, block,
    ...
    int ret = esp_partition_erase_range(default_littlefs_partition, block * bd->cfg->erase_size, bd->cfg->erase_size);
