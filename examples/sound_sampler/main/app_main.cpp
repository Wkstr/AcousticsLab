...
extern "C" void app_main()
{
    ...
    auto adpcm_encoder = core::EncoderADPCMIMA(nullptr, adpcm_buf_sz);
    int err = adpcm_encoder.error();
    if (err != 0)
    {
        std::cout << "ADPCM encoder error: " << err << std::endl;
        return;
    }
    auto base64_encoder = core::EncoderASCIIBase64(nullptr, base64_buf_sz);
    err = base64_encoder.error();
    if (err != 0)
    {
        std::cout << "Base64 encoder error: " << err << std::endl;
        return;
    }
    ...
    while (1)
    {
        ...
        std::cout << "SP: " << static_cast<int>(state.predictor) << " " << static_cast<int>(state.step_index) << " "
